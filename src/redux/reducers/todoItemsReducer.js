...
export default function todoItemsReducer(
  ...
  switch (action.type) {
        ...
        todos: map(state.todos, (todo) => {
          ...
          return todo.id === action.payload.todoId
            ? {
                ...
                todoList: [
                  ...todoList,
                  includes(todoList, todoListCategory)
                    ? null
                    : todoListCategory,
                ],
              }
        ...
        todos: map(state.todos, (todo) =>
          todo.todoList.includes(todolistTitle)
            ? {
                ...
                todoList: filter(
                  todo.todoList,
                  (todoCategory) => todoCategory !== todolistTitle
                ),
              }
            : todo
        ),
    ...
    case FILTER_TODOS:
      ...
      const foundItem = filter(state.filtered, (todo) =>
    ...
    case ORDER_TODOS_BY_DATE_CREATED:
      ...
      const newestTodos = orderBy(
        state.filtered,
        (todo) => new Date(todo.dateCreated),
        'desc'
      );
      const oldestTodos = orderBy(
        state.filtered,
        (todo) => new Date(todo.dateCreated),
        'asc'
      );
      const todosByDateCreated =
        sortActionDates === 'newest' ? newestTodos : oldestTodos;
      return {
        ...state,
